...
import { adminCreateRestaurant, adminToggleRestaurantStatus } from "@/features/admin/admin.functions";
...
function ManageRestaurants() {
  ...
  async function handleToggleStatus(id: string, currentStatus: boolean) {
    try {
      ...
      toast.success(!currentStatus ? 'تم تفعيل المطعم بنجاح.' : 'تم تعطيل المطعم بنجاح.');
      load();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-extrabold text-foreground">إدارة المطاعم</h1>
          <p className="text-sm text-muted-foreground">إضافة شركاء جدد وإدارة المطاعم الحالية في رام الله.</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={submit} className="space-y-4 rounded-3xl bg-card p-6 shadow-card border border-border">
          <div className="border-b border-border pb-4">
            <h2 className="text-lg font-extrabold">تسجيل مطعم جديد</h2>
            <p className="text-xs text-muted-foreground">إنشاء حساب مالك وملف تعريفي للمطعم.</p>
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-primary">معلومات المطعم</h3>
              <In label="اسم المطعم" value={form.restaurant_name} onChange={(v) => setForm({ ...form, restaurant_name: v })} required />
              <In label="نوع الطبخ" value={form.cuisine} onChange={(v) => setForm({ ...form, cuisine: v })} required />
              <div className="space-y-1">
                <span className="block text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">المنطقة (رام الله)</span>
                <select
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-primary">تفاصيل المالك</h3>
              <In label="الاسم الكامل للمالك" value={form.display_name} onChange={(v) => setForm({ ...form, display_name: v })} required />
              <In label="رقم التواصل" type="tel" value={form.phone_number} onChange={(v) => setForm({ ...form, phone_number: v })} />
              <div className="grid grid-cols-2 gap-2">
                <In label="بريد الدخول" type="email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} required />
                <In label="كلمة مرور مؤقتة" type="password" value={form.password} onChange={(v) => setForm({ ...form, password: v })} required />
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center gap-3 rounded-full bg-card px-4 py-3 shadow-sm border border-border">
        ...
        <input
          ...
          placeholder="ابحث عن المطاعم بالاسم..."
      ...
      <div className="hidden md:block rounded-2xl bg-card shadow-card overflow-hidden border border-border">
        <table className="w-full text-right text-sm">
          <thead className="bg-secondary/50 text-xs text-muted-foreground border-b border-border">
            <tr>
              <th className="px-5 py-4 font-semibold">المطعم</th>
              <th className="px-5 py-4 font-semibold">التواصل والمنطقة</th>
              <th className="px-5 py-4 text-center font-semibold">الوجبات</th>
              <th className="px-5 py-4 text-left font-semibold">المبيعات</th>
              <th className="px-5 py-4 text-center font-semibold">الحالة</th>
              <th className="px-5 py-4 text-center font-semibold">إجراءات</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {list.map((r) => (
              <tr key={r.id} className="hover:bg-muted/30">
                ...
                <td className="px-5 py-4 text-left font-bold text-primary">₪{r.totalSales.toFixed(2)}</td>
            ...
            {list.length === 0 && (
              <tr><td colSpan={6} className="py-10 text-center text-sm text-muted-foreground">لم يتم العثور على مطاعم.</td></tr>
            )}
          </tbody>
        </table>
      </div>
          ...
          <li key={r.id} className="rounded-2xl bg-card p-4 shadow-sm border border-border">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 border-t border-border pt-3">
              <div className="text-center">
                <p className="text-[10px] uppercase text-muted-foreground">الوجبات</p>
                <p className="font-bold">{r.itemCount}</p>
              </div>
              <div className="text-center border-r border-border">
                <p className="text-[10px] uppercase text-muted-foreground">المبيعات</p>
                <p className="font-bold text-primary">₪{r.totalSales}</p>
              </div>
              <div className="text-center border-r border-border">
                <p className="text-[10px] uppercase text-muted-foreground">الحالة</p>
                <span className={`text-[11px] font-bold ${r.active ? "text-success" : "text-destructive"}`}>{r.active ? "نشط" : "غير نشط"}</span>
              </div>
            </div>
          </li>
        ))}
