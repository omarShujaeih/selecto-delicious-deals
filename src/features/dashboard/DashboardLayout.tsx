...
const restaurantNav = [
  ...
  { to: "/dashboard/offers", label: "قائمة الطعام", icon: ListChecks },
  { to: "/dashboard/offers/new", label: "إضافة وجبة", icon: PlusCircle },
...
const adminNav = [
  { to: "/dashboard/admin", label: "لوحة التحكم", icon: Shield, exact: true },
  { to: "/dashboard/admin/restaurants", label: "المطاعم", icon: Store },
  { to: "/dashboard/admin/offers", label: "الوجبات", icon: ListChecks },
];
...
function DashboardLayout() {
  ...
  if (loading) {
    return <div className="grid min-h-dvh place-items-center text-sm text-muted-foreground">جاري التحميل...</div>;
  }
  if (!user) return null;
    ...
    return (
      <div className="mx-auto grid min-h-dvh max-w-md place-items-center px-6 text-center">
        <div className="space-y-3">
          <h1 className="font-display text-xl font-extrabold">لا يوجد صلاحية للوصول</h1>
          <p className="text-sm text-muted-foreground">
            حسابك ليس له صلاحية مطعم أو مسؤول. قم بالتسجيل كمطعم لإدارة الوجبات.
          </p>
      ...
      <aside className="mb-4 shrink-0 rounded-2xl bg-card p-4 shadow-card lg:mb-0 lg:w-60 lg:self-start">
        <div className="flex items-center gap-2 px-1 pb-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">{isAdmin ? "لوحة تحكم المسؤول" : "مطعم"}</p>
          </div>
        </div>
        <nav className="space-y-3">
          {isRestaurant && <NavGroup title="المطعم" items={restaurantNav} isActive={isActive} />}
          {isAdmin && <NavGroup title="المسؤول" items={adminNav} isActive={isActive} />}
        </nav>
        <button
          onClick={signOut}
