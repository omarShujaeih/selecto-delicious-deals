...
import { createOffer } from "@/features/dashboard/restaurant.functions";
import { fetchMyRestaurant, toCustomerPrice } from "@/features/offers/offers.service";
...
function AddOffer() {
  ...
  useEffect(() => {
    if (!user) return;
    fetchMyRestaurant(user.id).then((restaurant) => {
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  const { error: uploadError } = await supabase.storage.from("selecto_images").upload(fileName, file);
                  if (uploadError) throw uploadError;
                  const { data } = supabase.storage.from("selecto_images").getPublicUrl(fileName);
