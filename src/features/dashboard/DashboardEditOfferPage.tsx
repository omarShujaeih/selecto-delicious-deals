...
import { fetchMyRestaurant, fetchOfferById, toCustomerPrice } from "@/features/offers/offers.service";
import { updateOffer } from "@/features/dashboard/restaurant.functions";
...
function EditOffer() {
  ...
  const customerPrice = toCustomerPrice(discounted);
  const pct = original > 0 ? Math.round(((original - customerPrice) / original) * 100) : 0;
  ...
  useEffect(() => {
    ...
    fetchOfferById(id).then((o) => {
      if (o) {
        ...
        setDiscounted(o.restaurantPrice ?? o.discountedPrice);
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    if (!restaurantId) {
      toast.error("لا يوجد مطعم مرتبط بحسابك.");
      return;
    }
    setBusy(true);
    try {
      ...
      toast.error(err.message || "فشل في حفظ العرض");
    } finally {
      setBusy(false);
    }
  }
  ...
  const previewOffer = {
    ...
    discountedPrice: customerPrice || 0,
    restaurantPrice: discounted || 0,
    image: image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=800&q=80",
    isPublic: active,
  };

  const commission = customerPrice - discounted;

  return (
    <div className="space-y-6" dir="rtl">
      <h1 className="font-display text-xl font-extrabold">تعديل الوجبة</h1>

      <div className="space-y-2">
        <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">معاينة مباشرة</p>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="space-y-3 rounded-2xl bg-card p-4 shadow-card">
          <Field label="اسم الوجبة" value={name} onChange={(e) => setName(e.currentTarget.value)} required />
          <div>
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">صورة</p>
            <input
              type="file"
              accept="image/*"
              onChange={async (e) => {
                ...
                try {
                  ...
                  const { error: uploadError } = await supabase.storage.from("selecto_images").upload(filePath, file);
                  if (uploadError) throw uploadError;
                  const { data } = supabase.storage.from("selecto_images").getPublicUrl(filePath);
            ...
            {image && image.startsWith("http") && (
              <img src={image} alt="Preview" className="mt-2 h-20 w-20 rounded-md object-cover" />
            )}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="التصنيف" value={category} onChange={(e) => setCategory(e.currentTarget.value)} />
            <Field label="صالح حتى" value={validUntil} onChange={(e) => setValidUntil(e.currentTarget.value)} />
          </div>
          <Field
            label="وقت الاستلام"
          ...
          <div className="rounded-xl bg-secondary/50 p-3 mt-2 text-sm">
            <p className="font-semibold mb-1">تفاصيل التسعير:</p>
            <div className="flex justify-between text-muted-foreground">
              <span>مستحقاتك:</span>
              <span>₪{discounted.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-muted-foreground">
              <span>عمولة سيليكتو (20%):</span>
